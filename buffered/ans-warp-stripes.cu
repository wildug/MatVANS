...
extern __global__ void mat_vec_ans_thread_per_row(
    ...
    const uint32_t *__restrict__ ppf_prob)
{
    ...
    mat_vec_ans_thread_per_row<<<grid, block, in_dim * sizeof(int8_t)>>>(
...
void print_data_summary(std::span<const uint32_t> compressed, std::span<const int8_t> initial_vector, std::span<const float> scales)
{
    ...
    std::cout << "Read " << scales.size() << " matrices of dimensions "
              ...
              << "total compressed size: " << std::fixed << std::setprecision(2) << megabytes << " MB."
              << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    auto all_warp_offsets = file.get_data<uint32_t, 2>("all_warp_offsets", std::move(std::array{num_matrices, dim / WARP_SIZE}));
    ...
    auto num_warps_per_matrix = dim / WARP_SIZE; // Assuming one thread per matrix row
