...
__global__ void mat_vec_ans_warp_per_row(
    ...
    float scale)
{
    ...
    const int row = global_thread_id / WARP_SIZE;
    const __align__(16) uint4 *__restrict__ cursor = compressed + warp_offsets[row];
    ...
    cursor += WARP_SIZE;                                           // Advance `cursor` past the data we just read.
    ...
    for (int j = 0; j < in_dim_div4; j += WARP_SIZE)
    {
        ...
        const char4 b = in_vec_shared_char4[j + lane];
    ...
    // Accumulate `acc` across all threads in the warp using warp shuffle operations
    for (int offset = WARP_SIZE / 2; offset > 0; offset /= 2)
    {
        acc += __shfl_down_sync(0xFFFFFFFF, acc, offset);
    }
    // Lane 0 now has the final result for this row.
    if (lane == 0)
    {
