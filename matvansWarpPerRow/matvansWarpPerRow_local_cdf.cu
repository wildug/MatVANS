...
__constant__ uint8_t d_cdf_data[257];
...
__global__ void decmpressAndMultiply(int32_t* dst, int8_t* vec,
    ...
    extern __shared__ uint8_t cdf_shared[]; // store cdf in shared memory
    ...
    for (int j = 0; j < cols/WARP_SIZE; j++){
            ...
            prob = (d_cdf_data[r+1] - d_cdf_data[r]) & 0xFF; // modulo 2**8 to ensure it fits in a uint8
            head = (head >> 8) * prob  + (quantile -d_cdf_data[r]);
...
int main() {
    ...
    for (int l=0; l< NUM_ITERATIONS; l++){ // outer loop for benchmarking
        ...
        for (int k = 0; k<num_matrices; k++){
            ...
            // cudaMemcpy(d_cdf_data, matrix.cdf_data,sizeof(uint8_t)*(matrix.G +1), );
            cudaMemcpyToSymbol(d_cdf_data, matrix.cdf_data,sizeof(uint8_t)*(matrix.G +1));
