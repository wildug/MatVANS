...
parser.add_argument('-w', type=int, default=4096, help="The number of columns and rows for each matrix  (default is 4096)")
parser.add_argument('-n', type=int, default=20, help="The number of matrices (default is 20)")
parser.add_argument('-e','--entropy', type=int, default=5, help="The upper bound for the entropy (default is 5)")
parser.add_argument('-u', '--uncompressed', action='store_true', help="Set to true for uncompressed mode (default is False)")
...
# Print the values (or proceed with your code logic)
print(f"w: {w}, n: {n}, entropy: {entropy}")
...
final_result = v_int8
print("Result vector, ", list(v_int8))
...
with Pool(cpu_count()-1) as pool:
        encoded_matrices = list(
            tqdm(pool.map(encode_matrix, quantized_matrices),
                 total=len(quantized_matrices),
                 desc="Encoding matrices")
        )
...
with open(f'compressed_matrices_{w}_{entropy}bit_count{n}.bin', 'wb') as file:
